...
function AdminOrders() {
  ...
  const reload = async () => {
    try {
      setLoading(true);
      const data = await list({ data: { refreshKey: Date.now() } });
      ...
      <div className="flex justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={reload}
            disabled={loading}
            className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium text-foreground disabled:opacity-50"
          >
            Refresh
          </button>
