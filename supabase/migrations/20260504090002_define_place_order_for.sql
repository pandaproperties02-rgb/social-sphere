-- Define place_order_for for API usage
...
DECLARE
  ...
  _order_id bigint;
BEGIN
  -- Check if user has admin role or is the user
  ...
  INSERT INTO public.orders(user_id, service_id, link, quantity, charge, remains, status)
    VALUES (_user_id, _service_id, _link, _quantity, _charge, _quantity, 'in_progress')
    RETURNING id INTO _order_id;
  INSERT INTO public.wallet_transactions(user_id, amount, type, reference)
    VALUES (_user_id, -_charge, 'order', 'order#' || _order_id);
  RETURN _order_id;
END; $$;

-- Grant execute to authenticated
GRANT EXECUTE ON FUNCTION public.place_order_for(uuid, bigint, text, integer) TO authenticated;